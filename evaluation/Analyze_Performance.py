...
def analyze_performance(VIDEO, MODEL):
    ...
    if path.exists(face_trigger_path):
        ...
        plt.fill_between(range(0, 169), df_face_trigger['Face_Trigger'], step="pre", alpha=0.3, color='red')
    ...
    if path.exists(age_trigger_path):
        ...
        plt.fill_between(range(0, 169), df_age_trigger['Age_Trigger'], step="pre", alpha=0.3, color='blue')
    ...
    if path.exists(gender_trigger_path):
        ...
        plt.fill_between(range(0, 169), df_gender_trigger['Gender_Trigger'], step="pre", alpha=0.3, color='blue')
    ...
    if path.exists(blur_pixelate_path):
        ...
        plt.fill_between(range(0, 169), df_blur_pixelate['Blur_Pixelate'], step="pre", alpha=0.35, color='green')
    ...
    if path.exists(fill_area_path):
        ...
        plt.fill_between(range(0, 169), df_fill_area['Fill_Area'], step="pre", alpha=0.35, color='green')
...
analyze_performance('video_1', 'model_1')
